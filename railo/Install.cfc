<cfcomponent displayname="Install" output="false">
  <!--- Extension name --->
  <cfset variables.name = "Twitter Gateway" />
  ...
  <cfset variables.drivers = array(
    "TwitterUserStream.cfc"
  ) />

  <cfset variables.listeners = array(
    "TwitterUserStreamListener.cfc"
  ) />
  ...
  <!---
    Called from Railo to install the gateway
  --->
  <cffunction name="install" access="public" returntype="string" output="false">
    ...
    <!--- Update listeners --->
    <cfif NOT directoryExists("#serverPath#/components/railo/extension/gateway/twitter")>
      <cfdirectory action="create" directory="#serverPath#/components/railo/extension/gateway/twitter" />
    </cfif>
  ...
  <!---
    Called from Railo to update the gateway
  --->
  ...
  <!---
    Called from Railo to uninstall the gateway
  --->
  <cffunction name="uninstall" access="public" returntype="string" output="false">
    ...
    <cfloop array="#variables.listeners#" index="listener">
      <cfset removeFile("#serverPath#/components/railo/extension/gateway/twitter/#listener#", errors) />
    </cfloop>
    <cfdirectory action="delete" directory="#serverPath#/components/railo/extension/gateway/twitter" />
